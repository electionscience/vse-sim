...
from stratFunctions import truth
import random
import csv
import os
    ...
    def __init__(
        ...
        for m in methodsAndStrats:
            if issubclass(m, Method):
                ...
                for targetFunc in [select21, select31]:
                    fgs.extend(
                        [
                            (paramStrat(m.diehardBallot, intensity=i), targetFunc)
                            for i in m.diehardLevels
                        ]
                        + [
                            (paramStrat(m.compBallot, intensity=i), targetFunc)
                            for i in m.compLevels
                        ]
                    )
        ...
        for i in range(niter):
            ...
            for method, bgStrat, fgs in ms:
                result = method.threeRoundResults(
                    electorate, bgStrat, fgs, r1Media=r1Media, r2Media=r2Media
                )
...
def compareStrats(method, model, backgroundStrat, nvot, ncand, niter):
    pass
