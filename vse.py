...
def threeRoundResults(
    ...
    r0Results = Approval.results(
        [
            useStrat(voter, Approval.zeroInfoBallot, pickiness=pickiness)
            for voter in voters
        ]
    )
    ...
    backgroundBallots = [
        useStrat(voter, backgroundStrat, electabilities=electabilities, **bgArgs)
        for voter in voters
    ]
    ...
    r0Places = [
        sorted(r0Results, reverse=True).index(result) + 1 for result in r0Results
    ]
    r1Places = [
        sorted(r1Results, reverse=True).index(result) + 1 for result in r1Results
    ]

    constResults = dict(
        method=method.__name__,
        electorate=voters.id,
        backgroundStrat=backgroundStrat.__name__,
        numVoters=len(voters),
        numCandidates=len(voters[0]),
        magicBestUtil=max(totalUtils),
        magicWorstUtil=min(totalUtils),
        meanCandidateUtil=mean(totalUtils),
        bgArgs=bgArgs,
        r0ExpectedUtil=sum(
            p * u for p, u in zip(winProbs, totalUtils)
        ),  # could use electabilities instead
        r0WinnerUtil=totalUtils[r0Winner],
        r1WinProb=winProbs[r1Winner],
        r1WinnerUtil=totalUtils[r1Winner],
    )

    allResults = [
        makeResults(
            results=r0Results,
            totalUtil=totalUtils[r0Winner],
            probOfWin=winProbs[r0Winner],
            **constResults
        ),
        makeResults(
            results=r1Results,
            totalUtil=totalUtils[r1Winner],
            probOfWin=winProbs[r1Winner],
            winnerPlaceInR0=r0Places[r1Winner],
            **constResults
        ),
    ]
    ...
    for foregroundStrat, targetSelect, foregroundSelect, fgArgs in foregrounds:
        ...
        candToHelp, candToHurt = targetSelect(
            electabilities=electabilities, polls=polls, r0polls=electabilities
        )
        pollOrder = [
        ...
        lastVoter = foreground[fgSize - 1][0] if fgSize else [0.0] * len(r1Results)
        deciderUtilDiffs = [(lastVoter[winner] - lastVoter[r1Winner], nan, fgSize)]
        allUtilDiffs = [
            ([voter[0][winner] - voter[0][r1Winner] for voter in foreground], fgSize)
        ]
        while i < len(winnersFound):
            ...
            if (
                ...
                predeciderUtils = [0.0] * len(r1Results)
            else:
                predeciderUtils = foreground[threshold - 2][
                    0
                ]  # The one before the deciding voter
            deciderUtilDiffs.append(
                (
                    predeciderUtils[thisWinner] - predeciderUtils[r1Winner],
            ...
            allUtilDiffs.append(
                (
                    [
                        voter[0][thisWinner] - voter[0][r1Winner]
                        for voter in foreground[: threshold + 1]
                    ],
                    threshold,
                )
            )
            deciderMargUtilDiffs.append(
                (
                    deciderUtils[thisWinner] - deciderUtils[prevWinner],
                    threshold,
                    max(deciderUtils) - mean(deciderUtils),
                )
            )
            i += 1
        ...
        for i in range(len(deciderUtilDiffs) - 1):
            totalStratUtilDiff += (
                (deciderUtilDiffs[i][1] + deciderUtilDiffs[i + 1][0])
                / 2
            ...
            margStrategicRegret += sum(
                allUtilDiffs[i + 1][0][allUtilDiffs[i][1] : allUtilDiffs[i + 1][1]]
            )
            avgStrategicRegret += mean(allUtilDiffs[i + 1][0]) * (
                allUtilDiffs[i + 1][1] - allUtilDiffs[i][1]
            )
        ...
        partialResults.update(
            makePartialResults(
                [voter for voter, _, _ in foreground],
                winner,
                r1Winner,
                "",
                candToHelp,
        ...
        allResults.append(
            makeResults(
                results=results,
                fgStrat=foregroundStrat.__name__,
                fgTargets=targetSelect.__name__,
                fgArgs=fgArgs,
                winnerPlaceInR0=r0Places[winner],
                winnerPlaceInR1=r1Places[winner],
                probOfWin=winProbs[winner],
                numWinnersFound=len(winnersFound),
                totalUtil=totalUtil,
                pivotalUtilDiff=deciderMargUtilDiffs[0][0] / deciderMargUtilDiffs[0][2]
                if deciderMargUtilDiffs
                else 0,
                factionSize=fgSize,
                factionFraction=(deciderMargUtilDiffs[0][1] + 1) / fgSize
                if deciderMargUtilDiffs
                else None,
                deciderUtilDiffSum=sum(uDiff for uDiff, _, _ in deciderMargUtilDiffs),
                **partialResults
            )
        )
    return allResults
    ...
    def __init__(
        ...
        with multiprocessing.Pool(processes=7) as pool:
            results = pool.starmap(
                oneStepWorker, [args + (seed, i) for i in range(niter)]
            )
            for result in results:
                self.rows.extend(result)

        for row in self.rows:
            row["voterModel"] = str(model)
...
class CsvBatches(CsvBatch):
    def __init__(
        ...
        argsList = listProduct(
            listArgs
        )  # each entry is a list of arguments to be passed to one call of CsvBatch
        self.rows = []
        for a in argsList:
            self.rows.extend(
                CsvBatch(
