...
from methods import Approval, Irv, STAR, Minimax, Plurality, Score
...
class BlockApproval(makeBlock(Approval)):
    pass  # What's used in Fargo


class PBV(makeBlock(Irv)):
    pass  # Preferential BLock Voting, used in Utah


class BlockSTAR(makeBlock(STAR)):
    pass


class BlockMinimax(makeBlock(Minimax)):
    pass


class SNTV(makeBlock(Plurality)):
    pass  # not typically viewed as block voting, but this implements it
...
class AllocatedScore(STAR):
    ...
    def twoSlopeBallot(cls, utils, threshold=0.7, thresholdScore=2, **kw):
        ...
        for util in utils:
            if util > utilThreshold:
                ballot.append(
                    floor(
                        (cls.topRank - thresholdScore + 0.99)
                        * (util - utilThreshold)
                        / (top - utilThreshold)
                        + thresholdScore
                    )
                ...
                ballot.append(
                    floor(
                        (thresholdScore + 0.99) * (util - bot) / (utilThreshold - bot)
                    )
                )
        return ballot
    ...
    @classmethod
    def vaRangeBallot(
        cls, utils, electabilities, numWinners, exponent=2, boundConst=0.5, **kw
    ):
        """
        ...
        return cls.interpolatedBallot(
            adjustedUtils, 0, (highUtil - lowUtil) ** exponent
        )
    ...
    def truncatedBallot(cls, utils, threshold=0, exp=1, **kw):
        ...
        return [
            baseBallot[i]
            if (utils[i] - meanUtil) / (bestUtil - meanUtil) > threshold
            else 0
    ...
    def reweight(cls, ballots, winner, quota):
        ...
        for b in sortedBallots:
            ...
            if bScore == lastScore:
                ...
                scoreSupport = b.weight
        surplusFraction = (
            max(0, (totalSupport - quota) / scoreSupport) if scoreSupport > 0 else 0
        )
        for ballot in ballots:
...
class ASFinalRunoff(AllocatedScore):
    ...
    def pickWinner(cls, ballots, unelectedCands, quota, numWinners):
        ...
        for ballot in ballots:
            for c in unelectedCands:
                candTotals[c] += ballot[c] * ballot.weight
        (runnerUp, top) = sorted(range(len(ballots[0])), key=lambda i: candTotals[i])[
            -2:
        ]
        upset = sum(
            sign(ballot[runnerUp] - ballot[top]) * ballot.weight for ballot in ballots
        )
...
class ASRunoffs(AllocatedScore):
    ...
    def pickWinner(cls, ballots, unelectedCands, quota, numWinners):
        candTotals = [
            sum(b[c] * b.weight for b in ballots) if c in unelectedCands else -1
            for c in range(len(ballots[0]))
        ]
        (runnerUp, top) = sorted(range(len(ballots[0])), key=lambda i: candTotals[i])[
            -2:
        ]
...
class SequentialMonroe(AllocatedScore):
    ...
    def pickWinner(cls, ballots, unelectedCands, quota, numWinners):
        ...
        if len(bestCands) == 1:
            return bestCands[0]
        scores = [
            sum(ballot[c] * ballot.weight for ballot in ballots)
            if c in bestCands
            else -1
...
class SSS(AllocatedScore):
    ...
    def reweight(cls, ballots, winner, quota):
        ...
        for ballot in ballots:
            ballot.weight *= (
                1 - (1 - winnerSurplus / winnerTotal) * ballot[winner] / cls.topRank
            )
...
class S5H(SSS):
    ...
    def reweight(cls, ballots, winner, quota):
        ...
        while totalSupport < quota * cls.topRank and score > 1:
            ...
            totalSupport += scoreSupport
        surplusFraction = (
            max((totalSupport - quota * cls.topRank) / scoreSupport, 0)
            if scoreSupport
            else 0
        )
        for ballot in ballots:
            if ballot[winner] > score:
                ...
                ballot.weight *= (
                    1 - (1 - surplusFraction) * ballot[winner] / cls.topRank
                )
...
class TEA(AllocatedScore):
    ...
    def winnerSet(cls, ballots, numWinners):
        ...
        while threshold > 0 and len(winners) < numWinners:
            approvalCounts = [
                sum(
                    ballot.weight if ballot[i] >= threshold else 0
                    for ballot in wBallots
                )
                if i in unelectedCands
                else -1
            ...
            if any(c >= quota for c in approvalCounts):
                ...
                for cand in unelectedCands:
                    if approvalCounts[cand] >= quota:
                        sortedWeights = sorted(
                            b.weight
                            for b in wBallots
                            if b[cand] >= threshold and b.weight > 0
                        )
                        weightSum = 0
                        for i, w in enumerate(sortedWeights):
                            if (
                                weightSum + w * (len(sortedWeights) - i) >= quota
                                or w == sortedWeights[-1]
                            ):
                                costs[cand] = (quota - weightSum) / (
                                    len(sortedWeights) - i
                                )
                                break
                            else:
                ...
                if winner not in unelectedCands:
                    print(costs, approvalCounts)
                for b in wBallots:
        ...
        while len(winners) < numWinners:
            approvalCounts = [
                sum(ballot.weight if ballot[i] > 0 else 0 for ballot in wBallots)
                if i in unelectedCands
                else -1
                for i in range(numCands)
            ]
...
class MinimaxSTV(STV):
    ...
    def winnerSet(cls, ballots, numWinners):
        ...
        piles = [[] for _ in range(nCands)]
        while (
            len(winners) < numWinners - 1
            and len(winners) + len(candsLeft) > numWinners - 1
        ):
            cls.resort(ballotsToSort, candsLeft, piles)
            if newWinners := [
                c for c in candsLeft if sum(b.weight for b in piles[c]) >= quota
        ...
        compMatrix = [
            [
                float("inf")
                if i in winners or j in winners
...
class S5HtoSTV(S5H):
    ...
    def winnerSet(cls, ballots, numWinners):
        ...
        while any(
            sum(b[i] * b.weight for b in wBallots) >= quota * cls.topRank
            for i in unelectedCands
        ):
    ...
    def useSTV(cls, wBallots, numWinners, winners, quota, useForLastRound):
        ...
        while len(winners) < numToElect and len(winners) + len(candsLeft) > numToElect:
            candTotals = [
                sum(
                    b.weight / len(topCands)
                    for b, topCands in assignedBallots
                    if cand in topCands
                )
                if cand in candsLeft
                else -1
