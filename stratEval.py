...
def simpleStratTest(electorate, method, strats, numWinners, electabilities,
                    bgBallots, pollingError=0.2):
...
def methodStratLoop(voterModel, methods, strats, nvot, ncand, numWinners, bgStrat=None, bgStratArgs={},
                        pollingError=0.2, pollingMethod=None, pollingStrat=Approval.zeroInfoBallot,
...
class StratTest:
    def __init__(self, niter, voterModel, methods, strats, nvot, ncand, numWinners=1,
                    bgStrat=None, bgStratArgs={}, pollingError=0.2, pollingMethod=None,
                    pollingStrat=Approval.zeroInfoBallot, pollingStratArgs={'pickiness':0.7}):
        ...
        self.rowDicts = {keyTuple(m, strat, stat): []
                            for m in methods for strat in strats for stat in self.statNames}
        with multiprocessing.Pool(processes=7) as pool:
            results = pool.starmap(methodStratLoop,
                                    [(voterModel, methods, strats, nvot, ncand, numWinners,
                                      bgStrat, bgStratArgs, pollingError, pollingMethod,
                                      pollingStrat, pollingStratArgs)
            ...
            for m in methods:
                for i, strat in enumerate(strats):
                    for stat in self.statNames:
                        self.rowDicts[keyTuple(m, strat, stat)].append(row[m.__name__][i][stat])
        ...
        self.statNames = [""] if numWinners==1 else ['mean', 'max', 'median']
        self.esif = {keyTuple(m, strat, stat):
                        -(self.totals[keyTuple(m, strat, stat)] - self.totals[keyTuple(m, (m.abstain,{}), stat)])
                            /self.totals[keyTuple(m, (m.abstain,{}), stat)]
                     for m in methods for strat in self.strats for stat in self.statNames}
    ...
    def showESIF(self):
        for stat in self.statNames:
            ...
            for key, value in self.esif.items():
                ...
                print((key[0]+'\t' if self.multiMethods else "")
    ...
    def getUncertainty(self, m1, m2, s1, s2, stat=None):
        ...
        devOfTotal = std([a - b for a, b in zip(self.rowDicts[keyTuple(m1, s1, stat)], self.rowDicts[keyTuple(m2, s2, stat)])])*niter**0.5
        total = self.totals[keyTuple(m1, s1, stat)] - self.totals[keyTuple(m2, s2, stat)]
    ...
    def showScores(self):
        ...
        for key, value in self.esif:
            print((key[1]+'\t' if multiNames else "")
