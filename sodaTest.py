        ...
        while len(candidates):
            ...
            if (m[best, loser] > m[c, loser]) and (m[best, c] > m[loser, c]):
                ...
                if rival[0] is not None:
                    toWin = max(
                        (m[best, loser] - m[c, loser]), (m[best, c] - m[loser, c])
                    )
                    if rival[0][0] < toWin:
    ...
    def delegated(self, amounts, cantWin=None):
        ...
        if DEBUG:
            for i in range(self.n - 1):
                assert (
                    appr[0, dprefs[i]] >= appr[0, dprefs[i + 1]]
                ), "bullshit %i %i %s ... %s" % (
                    appr[0, dprefs[i]],
                    appr[0, dprefs[i + 1]],
                    appr,
        ...
        result.matrix = (
            self.matrix
            - self.oneMatrix(dprefs, self.delg[delegator])
            + self.appMatrix(appr)
        )
        return result
            ...
            for i in range(
                ...
                needed = max(
                    ...
                    min(
                        ...
                        - (
                            max(
                                self.matrix[:, curPrefs[i]][
                                    range(i + 1, worstWinnerIndex + 1), :
                                ]
                            )[0, 0]
                            - size
                        )
