...
class ThreeRoundResults:
    ...
    def __init__(
        ...
        self.r0Places = [
            sorted(self.r0Results, reverse=True).index(result) + 1
            for result in self.r0Results
        ]
        self.backgroundBallots = [
            useStrat(
                voter,
                self.backgroundStrat,
                electabilities=self.electabilities,
                **self.bgArgs,
            )
        ...
        self.constResults = dict(
            method=self.method.__name__,
            electorate=self.voters.id,
            backgroundStrat=self.backgroundStrat.__name__,
            numVoters=len(self.voters),
            bgArgs=self.bgArgs,
            numCandidates=self.numCands,
        )
        ...
        for foregroundStrat, targetSelect, foregroundSelect, fgArgs in self.foregrounds:
            self.foregroundStrat, self.targetSelect, self.fgArgs = (
                foregroundStrat,
                targetSelect,
                fgArgs,
            )
            polls = self.setR2Polls()
            candToHelp, candToHurt = targetSelect(
                electabilities=self.electabilities,
                polls=polls,
                r0polls=self.electabilities,
            )
            pollOrder = [
                cand for cand, poll in sorted(enumerate(polls), key=lambda x: -x[1])
            ]
            self.foreground = []  # (voter, ballot, eagernessToStrategize) tuples
            permbgBallots = []
            for id, voter in enumerate(voters):
                eagerness = foregroundSelect(
                    voter,
            ...
            self.foreground.sort(
                key=lambda v: -v[2]
            )  # from most to least eager to use strategy
            ...
            if (
                ...
                self.lastVoter = [0.0] * len(self.r1Results)
            self.initWinnerLoop()
            i = 1
            while i < len(self.winnersFound):
                self.thisWinner = self.winnersFound[i][0]
                self.threshold = self.stratThresholdSearch(
                    method,
                    self.thisWinner,
                    self.winnersFound[i][1],
                    permbgBallots,
                    fgBallots,
                    fgBaselineBallots,
                    self.winnersFound,
                ...
                self.prevWinner = self.winnerFunc(
                    permbgBallots
                    + fgBallots[: threshold - 1]
                    + fgBaselineBallots[threshold - 1 :]
                )
                if self.thisWinner == self.winner:
                    ...
                    prefix = "o"  # +str(i)
                self.partialResults.update(
                    self.makePartialResults(minfg, self.winner, self.r1Winner, prefix)
                )
                self.deciderUtils = self.foreground[self.threshold][
                    0
                ]  # The deciding voter
                if (
                    self.threshold == 0
                ):  # this shouldn't actually matter as we'll end up ignoring it anyway
                    # , so having the wrong utilities would be OK. But let's get it right.
                    self.predeciderUtils = [0.0] * len(self.r1Results)
                else:
                    self.predeciderUtils = self.foreground[self.threshold - 1][
                        0
                    ]  # The one before the deciding voter
    ...
    def round0(self):
        self.r0Results = self.pollingMethod.results(
            [
                useStrat(voter, self.pollingStrat, **self.pollingStratArgs)
                for voter in self.voters
            ]
        )
    ...
    def round1(self):
        ...
        self.r1Places = [
            sorted(self.r1Results, reverse=True).index(result) + 1
            for result in self.r1Results
        ]

    def addConstResults(self):
        self.constResults.update(
            dict(
                magicBestUtil=max(self.totalUtils),
                magicWorstUtil=min(self.totalUtils),
                meanCandidateUtil=mean(self.totalUtils),
                r0ExpectedUtil=sum(
                    p * u for p, u in zip(self.winProbs, self.totalUtils)
                ),  # could use electabilities instead
                r0WinnerUtil=self.totalUtils[self.r0Winner],
                r1WinProb=self.winProbs[self.r1Winner],
    ...
    def addRound0Results(self):
        r = self.makeResults(
            results=self.r0Results,
            totalUtil=self.totalUtils[r0Winner],
            probOfWin=self.winProbs[r0Winner],
            **self.constResults,
        )
    ...
    def addRound1Results(self):
        self.allResults.append(
            self.makeResults(
                results=self.r1Results,
                totalUtil=self.totalUtils[self.r1Winner],
                probOfWin=self.winProbs[self.r1Winner],
                winnerPlaceInR0=self.r0Places[self.r1Winner],
                **self.constResults,
            )
        )
    ...
    def initWinnerLoop(self):
        ...
        self.deciderUtilDiffs = [
            (
                self.lastVoter[self.winner] - self.lastVoter[self.r1Winner],
                nan,
                self.fgSize,
            )
        ]
        self.allUtilDiffs = [
            (
                [
                    voter[0][self.winner] - voter[0][self.r1Winner]
                    for voter in self.foreground
                ],
                self.fgSize,
            )
        ]
    ...
    def winnerLoopStats(self):
        self.deciderUtilDiffs.append(
            (
                self.predeciderUtils[self.thisWinner]
                - self.predeciderUtils[self.r1Winner],
                self.deciderUtils[self.thisWinner] - self.deciderUtils[self.r1Winner],
                self.threshold,
            )
        )
        self.allUtilDiffs.append(
            (
                [
                    voter[0][self.thisWinner] - voter[0][self.r1Winner]
                    for voter in self.foreground[: self.threshold + 1]
                ],
                self.threshold,
            )
        )
        self.deciderMargUtilDiffs.append(
            (
                self.deciderUtils[self.thisWinner] - self.deciderUtils[self.prevWinner],
                self.threshold,
            )
        )
    ...
    def finalStats(self):
        ...
        for i in range(len(self.deciderUtilDiffs) - 1):
            totalStratUtilDiff += (
                (self.deciderUtilDiffs[i][1] + self.deciderUtilDiffs[i + 1][0])
                / 2
                * (  # Use average over endpoints to interpolate average over range
                    self.deciderUtilDiffs[i + 1][2] - self.deciderUtilDiffs[i][2]
                )
            )
            margStrategicRegret += sum(
                self.allUtilDiffs[i + 1][0][
                    self.allUtilDiffs[i][1] : self.allUtilDiffs[i + 1][1]
                ]
            )
            avgStrategicRegret += mean(self.allUtilDiffs[i + 1][0]) * (
                self.allUtilDiffs[i + 1][1] - self.allUtilDiffs[i][1]
            )
        ...
        self.partialResults.update(
            self.makePartialResults(
                [voter for voter, _, _ in self.foreground],
                self.winner,
                self.r1Winner,
                "",
            )
        ...
        self.allResults.append(
            self.makeResults(
                ...
                fgStrat=self.foregroundStrat.__name__,
                fgTargets=self.targetSelect.__name__,
                fgArgs=self.fgArgs,
                winnerPlaceInR0=self.r0Places[self.winner],
                winnerPlaceInR1=self.r1Places[self.winner],
                probOfWin=self.winProbs[self.winner],
                numWinnersFound=len(self.winnersFound),
                totalUtil=self.totalUtil,
                pivotalUtilDiff=self.deciderMargUtilDiffs[0][0]
                if self.deciderMargUtilDiffs
                else 0,
                deciderUtilDiffSum=sum(uDiff for uDiff, _ in self.deciderMargUtilDiffs),
                **self.partialResults,
            )
        )
...
def winnerSetSample(numCands, numWinners):
    ...
    while winners[0] < numCands - numWinners:
        for i in range(numWinners - 1, -1, -1):
            if winners[i] < numCands - (numWinners - i) and (
                i == numWinners - 1 or winners[i + 1] - winners[i] > 1
            ):
                winners[i] += 1
    ...
    def addConstResults(self):
        ...
        self.constResults.update(
            dict(
                ...
                randBestUtil=mean(
                    max(v[c] for c in winnerSet)
                    for winnerSet in possOutcomes
                    for v in self.voters
                ),
                randMedianUtil=mean(
                    median(v[c] for c in winnerSet)
                    for winnerSet in possOutcomes
                    for v in self.voters
                ),
                highMeanUtil=max(
                    mean(mean(v[c] for c in winnerSet) for v in self.voters)
                    for winnerSet in possOutcomes
                ),
                highBestUtil=max(
                    mean(max(v[c] for c in winnerSet) for v in self.voters)
                    for winnerSet in possOutcomes
                ),
                highMedianUtil=max(
                    mean(median(v[c] for c in winnerSet) for v in self.voters)
                    for winnerSet in possOutcomes
                ),
                meanUtil=mean(v[w] for v in self.voters for w in self.round1Winners),
                maxUtil=mean(
                    max(v[w] for w in self.round1Winners) for v in self.voters
                ),
                medianUtil=mean(
                    medan(v[w] for w in self.round1Winners) for v in self.voters
                ),
            )
        )
