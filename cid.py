...
class candAffinity:
    ...
    def score(self, voter, candIndex):
        ...
        return (
            sum((sign(myUtil - u) + 1) * s for u, s in zip(voter, self.candSupport)) / 2
        )


class utilDeviation:
    def __init__(self, voters, **kw):
        pass
...
class weightedUtilDev:
    ...
    def __init__(
        self, voters, strat=Plurality.honBallot, method=Plurality, stratArgs=None
    ):
...
def influentialBlocs(
    voters,
    method,
    numWinners=1,
    utilChange=0.1,
    numBuckets=5,
    sorter=normalizedUtilDeviation,
    strat=None,
    stratArgs=None,
    ...
    if strat is None:
        ...
        basePollBallots = [
            pollingStrat(v, numWinners=numWinners, **pollingStratArgs) for v in voters
        ]
        pollErrors = [random.gauss(0, pollingError / 2) for _ in range(numCands)]
        polls = [
            min(1, max(0, r + e))
            for r, e in zip(pollingMethod.results(basePollBallots), pollErrors)
        ]
        ...
        baseBallots = [
            strat(
                v, polls=polls, electabilities=polls, numWinners=numWinners, **stratArgs
            )
            for v in voters
        ]
    ...
    for cand in range(numCands):
        ...
        sVoters, sBallots = zip(
            *sorted(zip(voters, baseBallots), key=lambda x: sorter.score(x[0], cand))
        )
        ...
        for b in range(numBuckets):
            if pollAfterPert:
                ...
                newPollBallots = [
                    pollingStrat(
                        v.addUtils(utilShifts),
                        numWinners=numWinners,
                        **pollingStratArgs,
                    )
                    for v in sortedVoters[
                        int(b * numVoters / numBuckets) : int(
                            (b + 1) * numVoters / numBuckets
                        )
                    ]
                ]
                noiselessPolls = pollingMethod.results(
                    [
                        pollingStrat(v, numWinners=numWinners, **pollingStratArgs)
                        for v in firstVoters
                    ]
                    + newPollBallots
                    + [
                        pollingStrat(v, numWinners=numWinners, **pollingStratArgs)
                ...
                polls = [
                    min(1, max(0, r + e)) for r, e in zip(noiselessPolls, pollErrors)
                ]
            newBallots = [
                strat(
                    v.addUtils(utilShifts),
            ...
            if pollAfterPert:
                ...
                ballots = (
                    sortedBallots[: int(b * numVoters / numBuckets)]
                    + newBallots
                    + sortedBallots[int((b + 1) * numVoters / numBuckets) :]
                )
            newWinners = method.winnerSet(ballots, numWinners=numWinners)
            if (cand in baseWinners and cand not in newWinners) or (
                cand not in baseWinners and cand in newWinners
            ):
                isIncentive[cand].append(1)
            else:
                isIncentive[cand].append(0)
...
class CID:
    ...
    def __init__(
        ...
        with multiprocessing.Pool(processes=7) as pool:
            results = pool.starmap(
                simOneElectorate, [args + (seed, i) for i in range(niter)]
            )
    ...
    def summarize(self):
        ...
        incentFracts = {
            name: [i * self.numBuckets / sum(incents) for i in incents]
            for name, incents in allIncents.items()
        }
    ...
    def chart(self, methodOnly=True):
        ...
        for name, data in incentFracts.items():
            ax.plot(
                [i / self.numBuckets for i in range(1, self.numBuckets + 1)],
                data,
                label=name,
            )
    ...
    def saveFile(self, baseName="cidResults", newFile=True):
        ...
        universalInfo = {
            ...
            "winners": self.nwinners,
            "model": str(self.model),
            "sorter": self.sorter.__name__,
            "numBuckets": self.numBuckets,
            "utilChange": self.utilChange,
            "pollingMethod": self.pollingMethod.__name__,
            "pollingError": self.pollingError,
        }
        fields = (
            ["name", "baseResult"]
            + list(range(self.numBuckets))
            + list(universalInfo.keys())
        )
...
def simOneElectorate(
    ...
    for method, strat, stratArgs in ms:
        allIncentives, baseWinners, baseResults = influentialBlocs(
            electorate,
            method,
            nwinners,
            utilChange,
...
def showChart(
    fileName, norm=1, methodOnly=True, forResult="all", percentages=True, wholeOnly=True
):
    with open(fileName) as file:
        reader = csv.DictReader(file)
        fig, ax = plt.subplots()
        for row in reader:
            if row["baseResult"] != forResult:
                continue
            ...
            ax.plot(
                [(i + 0.5) * 100 / buckets for i in range(buckets)], data, label=name
            )
        ax.set_xlabel("Voter's support for candidate")
        ax.set_ylabel("Candidate's incentive to appeal to voter")
        if percentages:
